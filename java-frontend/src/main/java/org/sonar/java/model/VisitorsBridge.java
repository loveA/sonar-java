...
public class VisitorsBridge {
  ...
  public void visitFile(@Nullable Tree parsedTree) {
    ...
    executableScanners.stream().filter(s -> s instanceof SymbolicExecutionVisitor || !(s instanceof IssuableSubscriptionVisitor))
  ...
  private static class ScannerRunner {
    ...
    public ScannerRunner(List<JavaFileScanner> executableScanners) {
      ...
      subscriptionVisitors = executableScanners.stream()
        .filter(s -> s instanceof IssuableSubscriptionVisitor)
        .map(s -> (SubscriptionVisitor) s).collect(Collectors.toList());
